 ...
 *
 * Parameters:
 * 	map<int,map<int,vector<Frag_Info_With_Cutting_Site>>> fragInteractionMap_local		the original dataset
 ...
 ***********************************************************************************************************************/
PoissMix& Get_Prox_Ligation_Frags(map< int, map< int, vector<Frag_Info_With_Cutting_Site> > > fragInteractionMap_local,vector<double>& poisVec, float pSampleSize, int n, ofstream& out, ofstream& iterationFile)
{
	...
	unsigned int cnt = 0;					// total elements from my_dataset
	...
	for(;chrIt != chrIt_end;++chrIt){
		...
		for(;CS_It != CS_It_end; ++CS_It){
			if (CS_It->second.size() != 0){
				// entire dataset in mix PM
				mix->x.push_back(CS_It->second.size());
				cnt++;
			}
		}
	}
	...
	// use % of dataset size
	sampleSize = mix->x.size() * pSampleSize;
	...
	for (i = 0; i < n; i++){
		...
		ts.x.clear();
		generateRandomDataset(ts, mix->x, sampleSize, cnt);
